...
class AddMovieComment extends Component {
    ...
    getCommentData = async (v) => {
      if (this.props.movieId) {
        try {
          const response = await axios.get(
            'https://finewf.club:9900/api/moviecomments/?format=json&page=' + this.state.page + '&page_size=' + count + '&movie=' + this.props.movieId
          )
    ...
    sendComment = async (value) => {
      try {
        ...
        const response = await axios.post(
          'https://finewf.club:9900/api/moviecomments/',
          {
            content: value,
