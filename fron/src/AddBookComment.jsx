...
class AddBookComment extends Component {
    ...
    getCommentData = async (v) => {
      if (this.props.bookId) {
        try {
          const response = await axios.get(
            'https://finewf.club:9900/api/bookcomments/?format=json&page=' + this.state.page + '&page_size=' + count + '&book=' + this.props.bookId
          )
    ...
    sendComment = async (value) => {
      try {
        ...
        const response = await axios.post(
          'https://finewf.club:9900/api/bookcomments/',
          {
            content: value,
