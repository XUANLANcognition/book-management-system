...
class FollowList extends Component {
  ...
  getProfileData = async (v) => {
    try {
      let url = ''
      url = 'https://finewf.club:9900/api/follow_relations/' + '?format=json' + '&page=' + this.page + '&page_size=' + count + '&follow=' + window.localStorage.getItem('user_id')
  ...
  handleFollowList = async (page) => {
    ...
    try {
      let url = ''
      url = 'https://finewf.club:9900/api/follow_relations' + '?format=json' + '&page=' + page + '&page_size=' + count + '&follow=' + window.localStorage.getItem('user_id')
      const response = await axios.get(url)
